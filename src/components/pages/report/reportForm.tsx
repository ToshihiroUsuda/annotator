...
export const ReportForm: React.FC<IReportFormProps> = (props) => {
  ...
  return (
    <div className="max-w-none">
      <Tabs
        defaultActiveKey="caseInfo"
        id="uncontrolled-tab-example"
        className="border-b border-transparent"
      >
        <Tab eventKey="caseInfo" title="Case Info1">
          <div className="overflow-y-scroll overflow-x-hidden p-3 bg-white/5 h-full">
            <PrivateInfoForm
        ...
        {LesionInfoKeys.map((lesionKey) => {
          return (
            <Tab
              ...
              title={lesionKey.charAt(0).toUpperCase() + lesionKey.slice(1)}
              disabled={props.report.exclusion || props.report.noLesions}
            >
              <div className="overflow-y-scroll overflow-x-hidden p-3 bg-white/5 h-full">
                <LesionInfoForm
                  formData={props.report.lesionInfo[lesionKey]}
                  schemaDir={props.schemaDir}
                  formSchemaName={constants.lesionInfoFormSchema}
...
const PartialReportForm = <T,>({
  ...
  useEffect(() => {
    (async () => {
      setFormSchema(
        await loadJson(props.schemaDir, props.formSchemaName, defaultFormSchema)
      );
      setUiSchema(
        await loadJson(props.schemaDir, props.uiSchemaName, defaultUiSchema)
      );
    })();
