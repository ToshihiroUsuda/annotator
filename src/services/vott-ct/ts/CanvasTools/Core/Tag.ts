...
export class Tag implements ITag {
    ...
    public static BuildFromJSON(data: ITag): Tag {
        if (data.color !== undefined) {
            ...
            return new Tag(
                data.name,
                new Color(new HSLColor((data.colorHue % 360) / 360.0, 1, 0.5)),
                data.id === undefined ? '' : data.id
            )
        }
        return new Tag(
    ...
    public get dispName(): string {
        return this.tagDispName
    }
    ...
    public get colorHighlight(): string {
        if (this.tagColorHighlight === '') {
            ...
            const highlight = new LABColor(
                lab[0] * 0.7,
                lab[1] * 0.7,
                lab[2] * 0.7
            )
            this.tagColorHighlight = highlight
                .toSRGB()
                .truncate()
                .toCSSString(0.4)
    ...
    public get colorText(): string {
        if (this.tagTextColor === '') {
            const rgb = this.colorObj.RGB
            const grayScale = rgb.r * 0.299 + rgb.g * 0.587 + rgb.b * 0.144
            this.tagTextColor = grayScale > 0.5 ? '#000' : '#fff'
    ...
    constructor(
        ...
        this.tagName = name
        this.tagDispName = dispName
        this.tagTitle = title

        if (typeof color === 'number') {
            this.colorObj = new Color(
                new HSLColor((color % 360) / 360.0, 1, 0.5)
            )
