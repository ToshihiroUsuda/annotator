...
export class RegionData implements IRegionData, IMovable, IResizable {
     ...
     * @returns A new `RegionData` object
     */
    public static BuildRectRegionData(
        x: number,
        y: number,
        width: number,
        height: number
    ...
    public move(arg1: number | IPoint2D, arg2?: number): void {
        const oldx = this.x
        const oldy = this.y
    ...
    public scale(f1: number, f2?: number): void {
        ...
        this.regionPoints = this.regionPoints.map(
            (p) => new Point2D(p.x * xf, p.y * yf)
        )
    }
