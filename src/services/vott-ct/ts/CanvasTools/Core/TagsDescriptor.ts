...
export class TagsDescriptor {
    ...
    public static BuildFromJSON(data: ITagsDescriptor): TagsDescriptor {
        ...
        const s =
            data.secondary === undefined
                ? []
                : data.secondary.map((tag) => Tag.BuildFromJSON(tag))

        return new TagsDescriptor(p, s)
    }

    private allTags: Tag[] = []
    private primaryTag: Tag | null = null
     ...
     */
    constructor(primaryTag: Tag | null, secondaryTags: Tag[])
    constructor(arg1?: Tag | Tag[] | null, arg2: Tag[] = []) {
    ...
    public toString(): string {
        ...
        if (this.primaryTag !== null) {
            str +=
                this.primaryTag.title ||
                this.primaryTag.dispName ||
                this.primaryTag.name

            this.secondary.forEach((tag) => {
                str += ', ' + tag.dispName || tag.name
            })
        } else {
            this.secondary.forEach((tag) => {
                str += ', ' + tag.dispName || tag.name
