...
export class LABColor implements ILabColorPoint {
  ...
  public distanceTo(color: LABColor): number {
    ...
    const c2 = Math.sqrt(
      color.values[1] * color.values[1] + color.values[2] * color.values[2]
    );
    const deltaC = c1 - c2;
  ...
  public distanceTo_00(color: LABColor): number {
    ...
    const h1 =
      b1 === 0 && a1t === 0 ? 0 : (Math.atan2(b1, a1t) * rad2deg) % 360;
    const h2 =
      b2 === 0 && a2t === 0 ? 0 : (Math.atan2(b2, a2t) * rad2deg) % 360;
    ...
    const T =
      1 -
      0.17 * Math.cos((H - 30) * deg2rad) +
      0.24 * Math.cos(2 * H * deg2rad) +
      0.32 * Math.cos((3 * H + 6) * deg2rad) -
      0.2 * Math.cos((4 * H - 63) * deg2rad);

    const SL =
      1 + (K2 * (midL - 50)) / Math.sqrt(20 + (midL - 50) * (midL - 50));
    const SC = 1 + K1 * midCt;
    const SH = 1 + K2 * midCt * T;
    const RT =
      -2 *
      midC7Root *
      Math.sin(60 * Math.exp(-((H - 275) / 25) * ((H - 275) / 25)) * deg2rad);
