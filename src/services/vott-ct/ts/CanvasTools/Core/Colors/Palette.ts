...
export class Palette {
    private gamutCluster: Color[] | undefined
    ...
    public constructor(settings: IPaletteSettings) {
        this.settings = {
            lightness:
                settings.lightness === undefined
                    ? 0.65
                    : Math.max(0, Math.min(1, settings.lightness)),
            lightnessVariation:
                settings.lightnessVariation === undefined
                    ? 0
                    : Math.max(0, Math.min(1, settings.lightnessVariation)),
            minGrayness:
                settings.minGrayness === undefined
                    ? 0
                    : Math.max(0, Math.min(1, settings.minGrayness)),
            maxGrayness:
                settings.maxGrayness === undefined
                    ? 2
                    : Math.max(0, Math.min(2, settings.maxGrayness)),
            granularity:
                settings.granularity === undefined
                    ? 50
                    : Math.max(10, settings.granularity),
            abRange:
                settings.abRange === undefined
                    ? 1.3
                    : Math.max(0, Math.min(2, settings.abRange)),
        }
    ...
    private generateGamutCluster(): Color[] {
        ...
        cluster = cluster.filter((p) => {
            ...
            return (
                d >= this.settings.minGrayness && d <= this.settings.maxGrayness
            )
        })
        ...
        cluster.forEach((p) => {
            ...
            if (this.settings.lightnessVariation > 0) {
                lightness +=
                    this.settings.lightnessVariation * (Math.random() - 0.5)
