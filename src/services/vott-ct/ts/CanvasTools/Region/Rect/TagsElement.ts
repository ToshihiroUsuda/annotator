...
export class TagsElement extends TagsComponent {
    ...
    public redraw(rebuildTags: boolean = false) {
        ...
        if (this.tags) {
            window.requestAnimationFrame(() => {
                if (
                    this.tags.primary !== undefined &&
                    this.tags.primary !== null
                ) {
                    ...
                    if (rebuildTags) {
                        // this.primaryTagText.node.innerHTML = (this.tags.primary !== null) ? this.tags.primary.dispName || this.tags.primary.name : "";
                        this.primaryTagText.node.innerHTML = ''
                        this.textBox = this.primaryTagText.getBBox()
                    }

                    // const showTextLabel = ((this.textBox.width+10)*2 <= this.width)
                    //                        && ((this.textBox.height+5)*2 <= this.height);
                    const showTextLabel = false
                    if (showTextLabel) {
                        ...
                        this.primaryTagTextBG.attr({
                            height: Math.min(1, this.height),
                            width: Math.min(1, this.width),
                            x: this.x,
                            y: this.y,
                        })
                        this.primaryTagText.attr({
            ...
            if (tags.primary !== null) {
                this.styleMap = [
                    ...
                    },
                    {
                        ...
                        // style: `fill:${tags.primary.colorAccent};`,
                        style: `fill:${tags.primary.colorText};`,
                    },
                    {
    ...
    private buildOn(paper: Snap.Paper, tags: TagsDescriptor) {
        ...
        this.primaryTagRect = paper.rect(
            this.x,
            this.y,
