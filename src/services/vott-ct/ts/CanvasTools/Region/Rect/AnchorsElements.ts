...
export class AnchorsElement extends AnchorsComponent {
    ...
    private boneThickness: number = 0
    ...
    public redraw() {
        ...
        const [x, y, width, height] = [
            this.regionData.x,
            this.regionData.y,
            this.regionData.width,
            this.regionData.height,
        ]
        ...
        window.requestAnimationFrame(() => {
            tBone.attr({
                x,
                y: y - this.boneThickness / 2,
                width,
                height: this.boneThickness,
            })
            rBone.attr({
                x: x + width - this.boneThickness / 2,
                y,
                width: this.boneThickness,
                height,
            })
            bBone.attr({
                x,
                y: y + height - this.boneThickness / 2,
                width,
                height: this.boneThickness,
            })
            lBone.attr({
                x: x - this.boneThickness / 2,
                y,
                width: this.boneThickness,
                height,
            })
    ...
    protected buildBoneAnchors() {
        ...
        const tBone = this.createAnchorBone(
            this.paper,
            x,
            y,
            w,
        ...
        const rBone = this.createAnchorBone(
            this.paper,
            x + w,
            y,
            0,
        ...
        const bBone = this.createAnchorBone(
            this.paper,
            x,
            y + h,
            w,
        ...
        const lBone = this.createAnchorBone(
            this.paper,
            x,
            y,
            0,
    ...
    protected createAnchorBone(
        ...
        height: number,
        style?: string,
        thickness: number = AnchorsComponent.DEFAULT_GHOST_ANCHOR_RADIUS
    ): Snap.Element {
        ...
        if (width === 0) {
            ...
            throw Error(
                'Rect bones that are neither vertical or horizontal are not supported.'
            )
        }
    ...
    protected updateRegion(p: Point2D) {
        ...
        if (activeAnchor !== newAA) {
            ...
            if (newAA.length === 2) {
                this.activeAnchorIndex =
                    this.anchorPointStyles.indexOf(newAA) + 1
            } else {
                this.activeAnchorIndex = -(
                    this.anchorBoneStyles.indexOf(newAA) + 1
                )
            }
        ...
        const p1 = new Point2D(Math.min(x1, x2), Math.min(y1, y2)).boundToRect(
            this.paperRect
        )
        const p2 = new Point2D(Math.max(x1, x2), Math.max(y1, y2)).boundToRect(
            this.paperRect
        )
    ...
    protected getActiveAnchorPoint(e?: PointerEvent): Point2D {
        if (this.activeAnchorIndex > 0) {
            ...
            if (e !== undefined) {
                ...
                const boneBox =
                    this.anchorBones[-this.activeAnchorIndex - 1].getBBox()
