...
export class TagsElement extends TagsComponent {
        ...
        if (this.tags) {
            ...
            if (this.tags.secondary && this.tags.secondary.length > 0) {
                ...
                for (let i = 0; i < length; i++) {
                    ...
                    const x =
                        this.x +
                        this.boundRect.width / 2 +
                        (2 * i - length + 1) * size -
                        size / 2
                    const y =
                        this.y - size - TagsElement.DEFAULT_SECONDARY_TAG_DY
    ...
    private buildOn(paper: Snap.Paper, tags: TagsDescriptor) {
        this.primaryTagNode = paper.circle(
            this.x,
            this.y,
            TagsElement.DEFAULT_PRIMARY_TAG_RADIUS
        )
