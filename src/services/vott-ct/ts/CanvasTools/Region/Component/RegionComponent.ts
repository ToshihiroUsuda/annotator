...
import { ILockable } from '../../Interface/ILockable'
 ...
 */
export abstract class RegionComponent
    implements IHideable, IResizable, IMovable, IFreezable, ILockable
{
    /**
    ...
    public hide() {
        ...
        this.node.node.setAttribute('opacity', '0.2')
        this.isVisible = false
    }

    public blind() {
        this.node.node.setAttribute('visibility', 'hidden')
        this.node.node.setAttribute('opacity', '0.0')
        // this.node.node.setAttribute("visibility", "visible");
        // this.node.node.setAttribute("opacity", "0.3");
        this.isVisible = false
    }
    /**
    ...
    public show() {
        this.node.node.setAttribute('visibility', 'visible')
        this.node.node.setAttribute('opacity', '1.0')
        this.isVisible = true
    }
    ...
    public lock() {
        this.isLocked = true
        this.node.addClass('locked')
    }

    public unlock() {
        this.isLocked = false
        this.node.removeClass('locked')
    }
    /**
    ...
    public move(arg1: IPoint2D | number, arg2?: number): void {
        if (typeof arg1 === 'number' && typeof arg2 === 'number') {
            this.regionData.move(arg1, arg2)
        } else if (typeof arg1 === 'object') {
            this.regionData.move(arg1)
        }
        this.redraw()
    }
     ...
     */
    protected makeFreezable(
        f: (args: PointerEvent | KeyboardEvent) => void,
        bypass: boolean = false
    ) {
        return (args: PointerEvent | KeyboardEvent) => {
            if ((!this.isFrozen && !this.isLocked) || bypass) {
                f(args)
            }
        }
    }
