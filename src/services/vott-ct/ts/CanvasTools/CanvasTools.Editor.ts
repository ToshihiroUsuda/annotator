...
import {
    RegionUpdateFunction,
    RegionSelectionFunction,
} from './Interface/IRegionsManagerCallbacks'
import {
...
type ToolbarIconDescription =
    | {
          type:
              | ToolbarItemType.SELECTOR
              | ToolbarItemType.SWITCH
              | ToolbarItemType.TRIGGER
          action: string
          iconFile: string
          tooltip: string
          keycode: string
...
export class Editor {
    ...
    public static FullToolbarSet: ToolbarIconDescription[] = [
        ...
        },
        {
            ...
            keycode: '',
            actionCallback: (_action: string, rm: RegionsManager) => {
                rm.deleteAllRegions()
            },
            activate: false,
        },
    ...
    public onRegionInfo: RegionUpdateFunction = () => {}
    ...
    public onRegionContextMenu: RegionSelectionFunction = () => {}
    /**
    ...
    constructor(
        ...
        const rmCallbacks = {
            ...
            onRegionSelected: (id: string, multiselection: boolean = false) => {
            ...
            onRegionMove: (id: string, regionData: RegionData) => {
                ...
                regions.forEach((region) => {
                    if (region.id !== id) {
                        if (this.regionsManager.isDoubleCheck) {
                            this.regionsManager.blindRegionById(region.id)
                        } else {
                            this.regionsManager.hideRegionById(region.id)
                        }
                    }
                })
            ...
            onRegionMoveEnd: (id: string, regionData: RegionData) => {
                this.regionsManager.showAllRegions()
            ...
            onRegionInfo: (id: string, regionData: RegionData) => {
                if (typeof this.onRegionInfo === 'function') {
                    this.onRegionInfo(id, regionData)
                    this.areaSelector.disable()
                }
            },
            onRegionContextMenu: (
                ...
                if (typeof this.onRegionContextMenu === 'function') {
                    this.onRegionContextMenu(id, multiselection)
                }
            },
        }
        ...
        const asCallbacks = {
            onSelectionBegin: () => {
                ...
                if (this.regionsManager.isDoubleCheck) {
                    this.regionsManager.blindAllRegions()
                } else {
                    this.regionsManager.hideAllRegions()
                }
        ...
        this.mergedAPI = new Proxy(this, {
            get: (target, prop) => {
                ...
                if (prop in target) {
                    t = target
                    p = (t as Record<string, unknown>)[prop as string]
                ...
                if (typeof p === 'function') {
                    return (...args: unknown[]): unknown => {
                        return (p as (...args: unknown[]) => unknown).apply(
                            t,
                            args
                        )
                    }
    ...
    public addToolbar(
        container: HTMLDivElement,
        toolbarSet: ToolbarIconDescription[],
        iconsPath: string
    ) {
        ...
        if (
            this.zoomManager.isZoomEnabled &&
            toolbarSet === Editor.RectToolbarSet
        ) {
            toolbarSet = toolbarSet
                .concat(Editor.SeparatorIconGroupToolbar)
                .concat(Editor.ZoomIconGroupToolbar)
        }
    ...
    public async addContentSource(
        ...
        return this.filterPipeline
            ...
            .then(() => {
                ...
                this.resize(
                    this.editorContainerDiv.offsetWidth,
                    this.editorContainerDiv.offsetHeight
                )
     ...
     */
    public scaleRegionToSourceSize(
    ...
    private onZoom(zoomType: ZoomDirection): void {
        ...
        if (zoomData) {
            const scaledFrameWidth =
                (this.frameWidth / zoomData.previousZoomScale) *
                zoomData.currentZoomScale
            const scaledFrameHeight =
                (this.frameHeight / zoomData.previousZoomScale) *
                zoomData.currentZoomScale
    ...
    private zoomEditorToScale(
        ...
        if (!this.editorContainerDiv.offsetWidth) {
            this.editorContainerDiv = document.getElementsByClassName(
                'CanvasToolsContainer'
            )[0] as HTMLDivElement
            this.editorDiv = document.getElementsByClassName(
                'CanvasToolsEditor'
            )[0] as HTMLDivElement
        }
