...
export class PolylineSelector extends Selector {
     ...
     * 点を追加する距離
     */
    private static THRESHOLD_DISTANCE_FOR_ADD_POINT: number = 10
    ...
    private buildUIElements() {
        ...
        this.nextPoint = this.paper.circle(
            0,
            0,
            PolylineSelector.DEFAULT_SELECTOR_RADIUS
        )
    ...
    private addPoint(x: number, y: number) {
        ...
        const point = this.paper.circle(
            x,
            y,
            PolylineSelector.DEFAULT_POINT_RADIUS
        )
    ...
    // AnchorElementの修正のためにwrapしたaddPoint
    public addPointPublic(x: number, y: number) {
        this.addPoint(x, y)
        if (!this.isCapturing) {
    ...
    private onPointerLeave(
        ...
        if (!this.isCapturing) {
            ...
            const p = new Point2D(
                (e as PointerEvent).clientX - rect[0].left,
                (e as PointerEvent).clientY - rect[0].top
            )
    ...
    private onPointerDown(
        ...
        if ((e as PointerEvent).ctrlKey && !this.isDrawing) {
            this.isDrawing = true
        }
    }

    /**
     * ドラッグ終了時にサブミットするために設定
     */
    private onPointerUp() {
        if (this.isDrawing) {
            this.submitPolyline()
    ...
    private onPointerMove(
        ...
        window.requestAnimationFrame(() => {
            ...
            if (this.lastPoint != null && this.isDrawing) {
                const threshold =
                    PolylineSelector.THRESHOLD_DISTANCE_FOR_ADD_POINT
                const distance = p.squareDistanceToPoint(this.lastPoint)
    ...
    private getPolylinePoints(close: boolean = true, threshold: number = 5) {
        ...
        if (points.length >= 3 && close) {
            ...
            const distanceSquare =
                (fp.x - lp.x) * (fp.x - lp.x) + (fp.y - lp.y) * (fp.y - lp.y)
     ...
     */
    private onKeyUp(e: PointerEvent | MouseEvent | KeyboardEvent | WheelEvent) {
