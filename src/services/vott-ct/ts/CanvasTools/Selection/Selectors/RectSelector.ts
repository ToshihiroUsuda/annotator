...
export class RectSelector extends Selector {
     ...
     */
    private selectionModificator: SelectionModificator =
        SelectionModificator.RECT
    ...
    private buildUIElements() {
        ...
        this.selectionBox = new RectElement(
            this.paper,
            this.boundRect,
            new Rect(0, 0)
        )
    ...
    private onPointerLeave(
        ...
        window.requestAnimationFrame(() => {
            ...
            const p = new Point2D(
                e.clientX - rect[0].left,
                e.clientY - rect[0].top
            )
            ...
            if (!this.isTwoPoints) {
                ...
                if (typeof this.callbacks.onSelectionEnd === 'function') {
                    ...
                    this.callbacks.onSelectionEnd(
                        RegionData.BuildRectRegionData(x, y, w, h)
                    )
                }
            } else {
                if (this.capturingState) {
            ...
            if (!this.isTwoPoints) {
                if (this.capturingState) {
                    this.moveCross(
                        this.crossB,
                        p,
                        this.selectionModificator ===
                            SelectionModificator.SQUARE,
                        this.crossA
                    )
                    this.moveSelectionBox(
                        this.selectionBox,
