...
export function InvertFilter(
    ...
    const buffContext = buff.getContext('2d')
    if (!buffContext) {
        return Promise.reject(new Error('Failed to get buffer context'))
    }
...
export function BlurDiffFilter(factor: number): FilterFunction {
    ...
    return (canvas: HTMLCanvasElement) => {
        ...
        const bludData = buffContext.createImageData(buff.width, buff.height)
        ...
        for (let i = 0; i < pixelsNumber; i++) {
            ...
            const d = 0.2358 * dr + 0.07 * dg + 0.6742 * db
        ...
        buffContext.putImageData(bludData, 0, 0)
...
export class FilterPipeline {
    ...
    public applyToCanvas(
        ...
        let promise = Promise.resolve(canvas)
