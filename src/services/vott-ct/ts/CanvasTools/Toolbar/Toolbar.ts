...
export class Toolbar {
    ...
    public setSwitch(action: string, on: boolean) {
        ...
        if (switchIcon && switchIcon instanceof ToolbarSwitchIcon) {
            if (on) {
                switchIcon.select()
            } else {
                switchIcon.unselect()
    ...
    private recalculateToolbarSize(newIcon?: ToolbarIcon) {
        if (newIcon === undefined) {
            ...
            this.toolbarHeight =
                this.icons.length * (ToolbarIcon.IconHeight + this.iconSpace) +
                this.iconSpace
        } else {
            ...
            this.toolbarHeight =
                this.toolbarHeight + newIcon.height + this.iconSpace
        }
    }
    ...
    private findIconByKeycode(keycode: string): ToolbarIcon | undefined {
        return this.icons.find((icon) => {
            return (
                icon.description !== null &&
                icon.description?.keycode === keycode
            )
        })
    }

    private findIconByAction(action: string): ToolbarIcon | undefined {
        return this.icons.find((icon) => {
            return (
                icon.description !== null && icon.description?.action === action
            )
        })
    }
        ...
        window.addEventListener('keyup', (e) => {
            if (
                ...
                if (this.areHotKeysEnabled && !e.ctrlKey && !e.altKey) {
                    ...
                    if (icon !== undefined) {
                        if (
                            icon instanceof ToolbarSelectIcon ||
                            icon instanceof ToolbarSwitchIcon ||
